...
VeloPic 是一款面向 Windows 10/11 的本地图片与视频浏览整理工具。应用使用 WinUI 3 构建，支持大目录递归扫描、目录分组、虚拟化缩略图、筛选排序、视频预览、收藏与相册、壁纸管理，以及完全离线的图片智能分类。
...
- 一次递归扫描图片和视频，通过“图片 / 视频”标签切换当前媒体类型。
- 支持“平铺 / 按目录”两种浏览模式并记忆最后选择；平铺模式全局排序，按目录模式显示完整相对路径。
- 目录标题可独立折叠，也可一键全部折叠或展开；筛选和媒体标签切换后保留本次运行中的折叠状态。
- 图片支持 JPG、JPEG、PNG、WebP、BMP、GIF、TIFF、HEIC、HEIF、AVIF。
- 视频支持 MP4、M4V、MOV、AVI、WMV、MKV、WebM、MPEG、MPG、3GP、MTS、M2TS。
- 使用虚拟化分组网格按需加载缩略图，视频缩略图由 Windows 文件缩略图 API 提取，并叠加播放标记。
- 按文件名、分辨率、横屏/竖屏方向和当前目录中的文件类型组合筛选。
- 按修改时间、文件名或文件大小排序，支持升序、降序及状态持久化。
- 支持收藏、相册、壁纸收藏夹、最近图片和自定义分类；单张图片可添加多个标签，并支持批量添加或移除标签。
- 使用本地 ONNX 模型智能分类为风景、城市、人物、自然，不上传图片。
- 提供沉浸式图片查看器、滚轮缩放、拖拽平移和前后图片切换。
- 使用 WinUI 3 原生 `MediaPlayerElement` 在右侧暂停预览视频；双击视频可自动进入沉浸播放。
...
视频沉浸播放支持 `Esc` 返回以及 `←` / `→` 切换同一筛选结果中的上一、下一个视频。视频标签中，收藏、相册、标签、智能分类和壁纸操作不可用；这些能力只作用于图片。
...
└─ VeloPic.Core/
   ├─ MediaFileScanner            图片与视频安全递归扫描
   ├─ MediaLibraryQueryService    媒体类型筛选与组内排序
   ├─ MediaDirectoryGrouping      目录标题和分组生成
   ├─ MediaViewerNavigator        图片/视频查看序列与首尾循环
...
测试覆盖图片/视频扩展名识别、安全递归扫描、扫描取消、媒体类型筛选、组内排序、目录标题、图片与视频查看导航、主题解析、分辨率与横竖屏筛选、多标签与批量移除标签、集合服务、设置往返保存、集合删除持久化及原子保存。
...
- 文件扫描、分辨率读取、视频缩略图提取和智能分类均异步执行，避免阻塞界面线程。
...
- 视频播放依赖 Windows 已安装的系统编解码器；应用不会随安装包附带 HEVC 等可能需要单独授权或购买的扩展。无法解码的视频仍会显示占位图、文件信息和一次性错误说明。
